...
class SlskProtoThread(threading.Thread):
    ...
    def bind_listen_port(self):
        ...
        for listenport in range(int(self.portrange[0]), int(self.portrange[1]) + 1):
            try:
                ...
                log.add("Listening on port: %i", listenport)
    ...
    def server_connect(self, msg_obj):
        ...
        log.add("Connecting to %(host)s:%(port)s", {'host': ip_address, 'port': port})
    ...
    def server_disconnect(self):
        ...
        log.add(
            "Disconnected from server %(host)s:%(port)s", {
                'host': ip_address,
                'port': port
            })

        if self._server_relogged:
            log.add("Someone logged in to your Soulseek account elsewhere")
            self._server_relogged = False
    ...
    def set_server_timer(self):
        ...
        log.add("The server seems to be down or not responding, retrying in %i seconds",
    ...
    def connect_error(self, error, conn_obj):
        ...
        if conn_obj.sock is self.server_socket:
            ...
            log.add(
                "Cannot connect to server %(host)s:%(port)s: %(error)s", {
                    'host': server_address,
                    'port': port,
                    'error': error
                }
    ...
    def establish_outgoing_server_connection(self, conn_obj):
        ...
        log.add(
            "Connected to server %(host)s:%(port)s, logging in", {
                'host': addr[0],
                'port': addr[1]
            }
        )
