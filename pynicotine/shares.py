...
if importlib.util.find_spec("_gdbm"):
    ...
    log.add("Cannot find %(option1)s or %(option2)s, please install either one." % {
        "option1": "gdbm",
        "option2": "semidbm"
    })
    sys.exit()
...
class Scanner:
    ...
    def run(self):
        ...
        try:
            ...
            from pynicotine.tinytag import TinyTag
            ...
            if self.rescan:
                ...
                self.queue.put(("Rescanning shares…", None, None))
                self.queue.put(("%(num)s folders found before rescan, rebuilding…",
                ...
                self.queue.put(("Rescan complete: %(num)s folders found", {"num": len(new_files)}, None))
    ...
    def set_shares(self, share_type, files=None, streams=None, mtimes=None, wordindex=None):
        ...
        for source, destination in storable_objects:
            ...
            try:
                ...
                self.queue.put(("Can't save %(filename)s: %(error)s",
    ...
    def rescan_dirs(self, share_type, mtimes=None, files=None, streams=None, rebuild=False):
        ...
        for folder in shared_folders:
            ...
            try:
                ...
                self.queue.put(("Error while scanning folder %(path)s: %(error)s", {
                    'path': folder,
                    'error': error
                }, None))
            ...
            for entry in os.scandir(encode_path(folder, prefix=False)):
                ...
                if entry.is_file():
                    try:
                        ...
                        self.queue.put(("Error while scanning file %(path)s: %(error)s",
    ...
    def get_file_info(self, name, pathname, tinytag, file_stat=None):
        ...
        if size > 128:
            try:
                ...
                self.queue.put(("Error while scanning metadata for file %(path)s: %(error)s",
...
class Shares:
    ...
    def __init__(self, core, config, queue, network_callback=None, init_shares=True):
    ...
    def load_shares(cls, shares, dbs, reset_shares=False):
        ...
        log.add("Failed to process the following databases: %(names)s", {
            'names': '\n'.join(errors)
        })
        log.add(exception)

        if not reset_shares:
            log.add("Attempting to reset index of shared files due to an error. Please rescan your shares.")
            cls.load_shares(shares, dbs, reset_shares=True)
            return

        log.add("File index of shared files could not be accessed. This could occur due to several instances of "
    ...
    def send_num_shared_folders_files(self):
        ...
        try:
            ...
            log.add("Failed to send number of shared files to the server: %s", error)
        ...
        while scanner.is_alive():
            ...
            while not scanner_queue.empty():
                ...
                if isinstance(item, tuple):
                    ...
                    self.core.emit('shares_set_scan_progress', item)
#                    log.add("Rescan progress: %s", str(int(item * 100)) + " %")

                elif item == "indeterminate":
                    self.core.emit('shares_show_scan_progress')
                    self.core.emit('shares_set_scan_indeterminate')
    ...
    def _process_scanner(self, scanner, scanner_queue):
        ...
        self.core.emit('shares_hide_scan_progress')
    ...
    def get_shared_file_list(self, msg):
        ...
        log.add("User %(user)s is browsing your list of shared files", {'user': user})
    ...
    def folder_contents_request(self, msg):
        ...
        if not checkuser:
            message = self.core.ban_message % reason
            self.queue.append(slskmessages.MessageUser(username, "[Automatic Message] " + message))
        ...
        if checkuser:
            try:
                ...
                log.add("Failed to fetch the shared folder %(folder)s: %(error)s",
